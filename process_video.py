...
def _python_for_inference(project_root: Path) -> Path:
    """Prefer repo venv (../venv) so inference runs with torch even if `python` is system/Homebrew."""
    venv_dir = project_root.parent / "venv" / "bin"
...
def _assert_input_has_video(path: Path) -> None:
    ...
    if suf in audio_only:
        print(
            f"Error: {path.name} is audio-only. RobustVideoMatting needs a **video** file "
    ...
    if r.returncode != 0 or "video" not in line:
        print(
            f"Error: no video stream in {path}. Use a file with a video track.",
            file=sys.stderr,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Frames per second in GIF (default 30 ≈ smooth phone video; use 15–24 for smaller files).",
    )
    parser.add_argument(
        "--no-premium",
        action="store_true",
